...
Checker.prototype.emulateBrowser = function (profile, url, userId, callback) {
    ...
    headless(opt,function(err, childProcess, servernum) {
        ...
        var browser = new Browser({browserWidth: profile.config.width, browserHeight: profile.config.height, displayServer: servernum, browsermobProxy: { port: 8080}, trackNetwork: true});
        browser.open(url);
        browser.on('error', function(err) {
            console.log(err);
        });
        browser.on('har', console.log);

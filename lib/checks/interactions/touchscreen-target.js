        ...
        ,   components : new Array()
            ...
            if(checker.resume["meta-viewport"]){
                ...
                for(index in targets.size) {
                    if((targets.size[index].width < 48 || targets.size[index].height < 48) && targets.size[index].width != 0 && targets.size[index].height != 0) {
                        smallTargetsDetected.tag.push(targets.tag[index]);
                        smallTargetsDetected.size.width.push(targets.size[index].width);
                        smallTargetsDetected.size.height.push(targets.size[index].height);
                        smallTargetsDetected.location.push(targets.location[index]);
                        if(targets.outerHtml[index].length > 40){
                            smallTargetsDetected.outerHtml.push(targets.outerHtml[index].substring(0,39) + '...');
                        }else{

    ...
    browser.on('har', function (har) {
        if (har && har.log && har.log.entries) {
            ...
            for (var i = 0; i < har.log.entries.length ; i++) {
                ...
                if (isCompressableResponse(entry.response) && !isCompressedResponse(entry.response)) {
                    compressable.push(entry.request.url);
                }
            }
            if (compressable.length) {
                checker.report(sink, "resources-could-be-compressed", self.name, self.category, {number: compressable.length, compressable: compressable.join(", ")});
            }

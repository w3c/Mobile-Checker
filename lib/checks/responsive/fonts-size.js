        ...
        return driver.executeScript(function(args) {
            ...
            for (var i in data) {
                for (var j in data[i]) {
                    ...
                    data[i][j].getCssValue("font-size").then(
                        function(size) {
                            fonts.size.push(parseFloat(size.replace(
                                "px", "")));
                        });
            ...
            if (smallFontsDetected.size.length > 0) {
                checker.report(sink, "too-small-font-size", self.name,
                    self.category, smallFontsDetected);
                return;
            } else {

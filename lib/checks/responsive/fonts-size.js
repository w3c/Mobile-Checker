...
exports.check = function (checker) {
	...
	for (index in tagFonts.fontSize){
		...
		if (ftSize < 16) {
			console.log("&lt;" + tagFonts.tagName[index] + "&gt;");
			sink.emit('err', checker.l10n.message(checker.options.lang, this.category, this.name, "too-small-font-size", {"htmlTag": "&lt;" + tagFonts.tagName[index] + "&gt;"}));
		}

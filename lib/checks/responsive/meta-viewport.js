...
exports.check = function (checker) {
    ...
    if (metaviewports  === undefined || metaviewports.length === 0) {
        checker.report(sink, "no-viewport-declared", this.name, this.category);
        return;
    }
    if (metaviewports.length > 1) {
	   checker.report(sink, "several-viewports-declared", this.name, this.category);
    }
    ...
    for (var prop in parsedViewport.invalidValues) {
        checker.report(sink, "invalid-viewport-value", this.name, this.category, { property: prop, value: parsedViewport.invalidValues[prop], validValues: metaparser.expectedValues[prop].join(", ")});
    }
    if (!parsedViewport.validProperties["width"] && !parsedViewport.validProperties["initial-scale"]) {
        checker.report(sink, "content-viewport-missed", this.name, this.category);
    } else {
        if (parsedViewport.validProperties["width"]) {
           if (parsedViewport.validProperties["width"] === "device-width" || parsedViewport.validProperties["width"] === "device-height") {
                ...
                checker.report(sink, "hardcoded-viewport-width", this.name, this.category);
            }
        }
        ...
        if (parsedViewport.validProperties["user-scalable"] === "no") {
            checker.report(sink, "users-are-prevented-to-zoom", this.name, this.category);
        }
        ...
        if (nonstandardProperties.length) {
            checker.report(sink, "non-standard-viewport-parameter-declared", this.name, this.category);
        }
        if (unrecognizedProperties.length) {
            checker.report(sink, "unknow-viewport-parameter-declared", this.name, this.category);
        }
    }
}

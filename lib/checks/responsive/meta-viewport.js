                ...
                webdriver.promise.map(
                    ...
                    function(metaviewports) {
                        if (metaviewports === undefined ||
                            ...
                            checker.report(sink, "no-viewport-declared",
                                self.name, self.category);
                            return;
                        }
                        if (metaviewports.length > 1) {
                            checker.report(sink,
                                "several-viewports-declared", self.name,
                                self.category);
                        }
                        ...
                        for (var prop in parsedViewport.invalidValues) {
                            checker.report(sink,
                                "invalid-viewport-value", self.name,
                                self.category, {
                                    property: prop,
                        ...
                        if (!parsedViewport.validProperties["width"] &&
                            !parsedViewport.validProperties[
                                "initial-scale"]) {
                            checker.report(sink,
                                "content-viewport-missed", self.name,
                                self.category);
                        } else {
                            if (parsedViewport.validProperties["width"]) {
                            ...
                            if (parsedViewport.validProperties[
                                "user-scalable"] === "no") {
                                checker.report(sink,
                                    "users-are-prevented-to-zoom", self
                                    .name, self.category);
                            }
                            ...
                            var nonstandardProperties =
                                unknownProperties.filter(
                                    function(i) {
                                        return
                                            wellknownMetaviewportProperties
                                            .indexOf(i) !== -1;
                                    }
                                );

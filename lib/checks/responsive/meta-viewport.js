...
exports.check = function (checker) {
    ...
    for (var prop in parsedViewport.invalidValues) {
        sink.emit('err', checker.l10n.message(checker.options.lang, this.category, this.name, "5", { property: prop, value: parsedViewport.invalidValues[prop], validValues: metaparser.expectedValues[prop].join(", ")}));
        sink.emit('done');
    }

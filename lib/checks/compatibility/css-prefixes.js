        ...
        if (har && har.log && har.log.entries) {
            for (var i = 0; i < har.log.entries.length ; i++) {
                ...
                if (entry.response.status === 200) {
                    var media = mediaTypeParser.fromString(entry.response.content.mimeType);
                    var mediaType = mediaTypeName(media);
                    if (mediaType === "text/css") {
                        var url = entry.request.url;
                        var cssObj = cssParser.parse(entry.response.content.text, {silent: true});
                        for (var j = 0 ; j < cssObj.stylesheet.rules.length; j++) {
                            ...
                            if (rule.declarations) {
                                for (var k = 0; k < rule.declarations.length; k++) {
                                    ...
                                    if (hasprefix(decl.property)) {
                                        var unprefixed = unprefix(decl.property);
                                        if (!prefixedProperties[unprefixed]) {
                                            prefixedProperties[unprefixed] = {};
                                        }
                                        prefixedProperties[unprefixed][decl.property] = decl;
                                    } else {
                                        unprefixedProperties[decl.property] = decl;
                                    }
                                }
                            }
                            ...
                            for (k = 0 ; k < prefixed.length ; k++) {
                                ...
                                if (unprefixedProperties[prefixed[k]]) {
                                    prefixedProperties[prefixed[k]][prefixed[k]] = unprefixedProperties[prefixed[k]];
                                }
                                ...
                                for (var l = 1; l < prefixedNames.length; l++) {
                                    ...
                                    if (props[prefixedNames[l]].value !== ref) {
                                        misMatchFound = true;
                                        break;
                                    }
                                }
                                if (misMatchFound) {
                                    ...
                                    mismatchPrefixEquivalent[url].push({
                                        ...
                                        prop: prefixed[k],
                                        decls: values(props).map(stringify)
                                    });
                                }
                                var equivPrefixes = cssPrefixes[prefixed[k]] ? cssPrefixes[prefixed[k]].values : undefined;
                                if (equivPrefixes) {
                                    var missing = [];
	        ...
	        checker.report(sink, "missing-prefixes", self.name, self.category, {missingPrefixes: missingPrefixEquivalent});
            }
            if (Object.keys(mismatchPrefixEquivalent).length) {
	        checker.report(sink, "mismatching-prefixes", self.name, self.category, {mismatchingPrefixes: mismatchPrefixEquivalent});
            }
        }
    });
};

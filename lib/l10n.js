...
var messages = {
	...
	,	"performance.http-errors.favicon": {
			"title":"Favicon unnecessarily checked"
		,	"what":"The page does not have a favicon at ${url}, but the browser had to make a request to determine it."
		,	"why":"Favicon helps users quickly identify a particular page in a list of opened windows; unless told otherwise, browsers always make requests to /favicon.ico to obtain these favicons. When no favicon is present, this generates unnecessary network traffic."
		,	"how": "Set up a favicon on the domain, or specific to the page; otherwise, use link rel='icon' href='data:;base64,iVBORw0KGgo=' in the page to prevent the browser from doing the additional request."
		}
...
exports.report = function (category, name, id, data) {
    var template = fs.readFileSync(path.join(__dirname, 'issues/responsive/doc-width/err.ejs'), 'utf8');
    return ejs.render(template, data);

};

exports.message = function (lang, category, check, res, data, cb) {

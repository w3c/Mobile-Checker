...
function setViewPort (driver) {
	...
	driver.findElements(webdriver.By.css('meta[name="viewport"]')).then(function(viewportDecls){
		if(viewportDecls.length > 0){
			contentAttr = viewportDecls[viewportDecls.length - 1].getAttribute("content").then(function(content){
				contentAttr = content;
	                        console.log('pageSpeed:metaviewport:' + content);
			});
		}
	}).then(function(){
		if(contentAttr) {

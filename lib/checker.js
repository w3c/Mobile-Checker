...
Checker.prototype.check = function (options) {
    ...
    var resourcesRequested = new Array();
    var resources;
    var reporting = {
        settings : {
    ...
    phantomjs.stdout.on('data', function (data) {
        ...
        switch (buffer[0]) {
            ...
            case 'resource':
                ...
                resourcesRequested.push(buffer[1]);
                    /*resource = JSON.parse(buffer[1]);
                var fileName = url.parse(resource.url).pathname.split('/').pop();
                if (fileName.length == 0) {
                    fileName = "index.html";
                    resources[fileName] = resource;
                    resources[fileName].name = fileName;
                }
                else {
                    resources[fileName] = resource;
                    resources[fileName].name = fileName;

...
Checker.prototype.check = function (options) {
	...
	if(!options.url) return this.throw(options.events, "Without url there is nothing to check.");
    if(!options.checklist) return this.throw(options.events, "Without checklist there is nothing to check");
    if(!options.id) return this.throw(options.events, "Without id, the app can't generate screenshot");
    ...
    if(!options.profile || validProfiles.indexOf(options.profile) === -1) {
        options.profile = 'default';
    }
     var profile = require('./profiles/' + options.profile + '.json')
    ,	sink = options.events
    ,	checklist = options.checklist
    ,   resume = {ok : 0,   issues : 0}
    ,	self = this
    ;
...
Checker.prototype.emulateBrowser = function (profile, url, id, callback) {
    var self = this;
    var opt = {display: {width: profile.width, height: profile.height}};
...
Checker.prototype.report = function (sink, key, name, category, data) {
    ...
    if(key == "ok"){
        ...
        sink.emit('err', this.issueReport(key, name, category, data));
        sink.emit('done');
    }
};

Checker.prototype.issueReport = function (key, name, category, data) {
...
Checker.prototype.throw = function (sink, msg) {
    if(!sink) return console.error("[BOOM] No event sink with which to report system errors.\nAlso: " + msg);
